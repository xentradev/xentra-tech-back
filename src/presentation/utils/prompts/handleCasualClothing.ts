const clothesMap: Record<string, string> = {
  ...
  fancy_dress:
    "a glamorous fancy dress with an embellished bodice and flowing skirt",
  ...
  princess_dress:
    "a fitted, bejeweled bodice and a voluminous, floor-length skirt",
  ...
  superhero:
    "a superhero uniform with a form-fitting bodysuit, a cape, and boots",
  teacher:
    "a teacher outfit with a button-up blouse, a pencil skirt, and sensible heels",
  viking: "a viking costume with a helmet and fur",
  firefighter:
    "a firefighter uniform with a protective jacket, pants, and helmet",
  military:
    "a military uniform with camouflage pattern, including a jacket, pants, and boots",
  construction:
    "a construction worker outfit with a hard hat, reflective vest, and durable work pants",
  ...
  ninja:
    "a ninja costume with a black long-sleeved top, matching pants, and a sash belt",
  ...
  belly_dancer:
    "a belly dancer outfit with a decorated bra top and a flowing skirt",
  goth: "goth clothing with dark, edgy designs and accessories",
  pirate: "a pirate costume with a tricorn hat, eye patch, and boots",
  prisoner:
    "a prisoner uniform with black and white stripes and a prisoner number",
  ...
  builder:
    "construction clothing with durable work pants, a shirt, and a tool belt",
  monk: "a monk costume with a simple, long robe and a hood",
  samurai:
    "a samurai costume with traditional armor, including a kimono and a katana",
};
...
export const handleCasualClothing = (prompt: string): string => {
  let clothesInPrompt;
  keysArray.forEach((word) => {
    if (prompt.includes(word)) {
      clothesInPrompt = clothesMap[word];
    }
  });

  if (!clothesInPrompt) {
    let rnd = Math.random();
    let item = Math.floor(rnd * keysArray.length);
    return clothesMap[keysArray[item]];
  } else {
    return clothesInPrompt;
  }
};
